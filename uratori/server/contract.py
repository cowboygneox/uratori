...
class LibraryOut(BaseModel):
    """What compiled, by name and version.

    The versions are the review surface: a host that commits its own compiled
    artifact can assert these hashes match it, which is how "the server runs
    what I reviewed" becomes a check rather than a hope.
    """

    figures: list[DeclarationRef]
    readings: list[DeclarationRef]
    projections: list[DeclarationRef]
    summaries: list[DeclarationRef]
    indexes: int
    measures: int
